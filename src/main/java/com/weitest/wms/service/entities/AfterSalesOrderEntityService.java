...
import com.weitest.wms.repository.entities.AfterSalesOrderEntityMapper;
...
/**
* auto generate AfterSalesOrderEntity ServiceImpl
*
* @author sys
*/
@Service
public class AfterSalesOrderEntityService extends AbstractService {
    @Resource
    private AfterSalesOrderEntityMapper mapper;
    @Resource
    ...
    public AfterSalesOrderEntityService() {
        entityFieldNameTitleMap.put("id", "售后单号");
        entityFieldTitleNameMap.put("售后单号", "id");
        ...
        entityFieldNameTitleMap.put("responsible", "负责人");
        entityFieldTitleNameMap.put("负责人", "responsible");
        entityFiledColumnNameMap.put("responsible", "responsible");
        entityFieldNameTitleMap.put("total_amount", "总金额");
        entityFieldTitleNameMap.put("总金额", "total_amount");
        entityFiledColumnNameMap.put("total_amount", "totalAmount");
        entityFieldNameTitleMap.put("number_of_cargo_entries", "货物条目数量");
        entityFieldTitleNameMap.put("货物条目数量", "number_of_cargo_entries");
        entityFiledColumnNameMap.put("number_of_cargo_entries", "numberOfCargoEntries");
        entityFieldNameTitleMap.put("notes", "备注");
        entityFieldTitleNameMap.put("备注", "notes");
        entityFiledColumnNameMap.put("notes", "notes");
        entityFieldNameTitleMap.put("id_tag", "编号");
        entityFieldTitleNameMap.put("编号", "id_tag");
        entityFiledColumnNameMap.put("id_tag", "idTag");
        entityFieldNameTitleMap.put("order_dateTime", "订单时间");
        entityFieldTitleNameMap.put("订单时间", "order_dateTime");
        entityFiledColumnNameMap.put("order_dateTime", "orderDateTime");
        entityFieldNameTitleMap.put("status", "售后订单状态");
        entityFieldTitleNameMap.put("售后订单状态", "status");
    ...
    **/
    ...
    public AfterSalesOrderEntity create(AfterSalesOrderEntity entity) {
        if (null == entity) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<AfterSalesOrderEntity> batchCreate(List<AfterSalesOrderEntity> entities) {
        ...
        // fill default value
        for (AfterSalesOrderEntity entity : entities) {
            paramValidate(entity);
        }
    ...
    **/
    @Transactional
    public AfterSalesOrderEntity update(AfterSalesOrderEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "AfterSalesOrderEntity");
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<AfterSalesOrderEntity> batchUpdate(List<AfterSalesOrderEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
        // updateFields为null时，默认全量更新
        List<AfterSalesOrderEntity> updateEntities = new ArrayList<>(entities.size());
        for (AfterSalesOrderEntity entity : entities) {
    ...
    **/
    ...
    public long delete( Long id ) { 
        ...
        AfterSalesOrderEntity entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("AfterSalesOrder", "AfterSalesOrder", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<AfterSalesOrderEntity>  entities = mapper.selectList(inQueryFilter);
        for (AfterSalesOrderEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
     ...
     * auto gen get method
     **/
    public AfterSalesOrderEntity get( Long id ) { 
        if ( id == null ) { 
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
        }
    ...
    **/
    public List<AfterSalesOrderEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<AfterSalesOrderEntity> data = ExcelUtil.read(file.getInputStream(), type, AfterSalesOrderEntity.class, entityFieldTitleNameMap);
            batchCreate(data);
            return "ok";
        } catch (Exception e) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<AfterSalesOrderEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, AfterSalesOrderEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + AfterSalesOrderEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
            throw new HttpCodeException(500, e);
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public AfterSalesOrderEntity createOrUpdate(AfterSalesOrderEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, AfterSalesOrderEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            AfterSalesOrderEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                paramValidate(entity);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(AfterSalesOrderEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
