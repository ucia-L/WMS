...
import com.weitest.wms.repository.entities.WarehouseEntityMapper;
...
/**
* auto generate WarehouseEntity ServiceImpl
*
* @author sys
*/
@Service
public class WarehouseEntityService extends AbstractService {
    @Resource
    ...
    public WarehouseEntityService() {
        entityFieldNameTitleMap.put("id", "主键");
        entityFieldTitleNameMap.put("主键", "id");
        entityFiledColumnNameMap.put("id", "id");
        entityFieldNameTitleMap.put("name", "仓库名称");
        entityFieldTitleNameMap.put("仓库名称", "name");
        entityFiledColumnNameMap.put("name", "name");
        entityFieldNameTitleMap.put("id_tag", "仓库编号");
        entityFieldTitleNameMap.put("仓库编号", "id_tag");
        entityFiledColumnNameMap.put("id_tag", "idTag");
        entityFieldNameTitleMap.put("createdTime", "仓库创建时间");
        entityFieldTitleNameMap.put("仓库创建时间", "createdTime");
        entityFiledColumnNameMap.put("createdTime", "createdTime");
        entityFieldNameTitleMap.put("createdBy", "仓库创建者");
        entityFieldTitleNameMap.put("仓库创建者", "createdBy");
        ...
        entityFieldNameTitleMap.put("balance", "余额");
        entityFieldTitleNameMap.put("余额", "balance");
        entityFiledColumnNameMap.put("balance", "balance");
        entityFieldTitleNameMap.put("capacity", "capacity");
        entityFiledColumnNameMap.put("capacity", "capacity");
        entityFieldTitleNameMap.put("current_quantity", "current_quantity");
        entityFiledColumnNameMap.put("current_quantity", "currentQuantity");
    }
    ...
    **/
    ...
    public WarehouseEntity create(WarehouseEntity entity) {
        ...
        entity.setCapacity(getOrDefault(entity.getCapacity(), 10000L));
        entity.setCurrent_quantity(getOrDefault(entity.getCurrent_quantity(), 1000L));
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<WarehouseEntity> batchCreate(List<WarehouseEntity> entities) {
    ...
    **/
    @Transactional
    public WarehouseEntity update(WarehouseEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "WarehouseEntity");
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<WarehouseEntity> batchUpdate(List<WarehouseEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
        // updateFields为null时，默认全量更新
        List<WarehouseEntity> updateEntities = new ArrayList<>(entities.size());
        for (WarehouseEntity entity : entities) {
    ...
    **/
    ...
    public long delete( Long id ) { 
        ...
        WarehouseEntity entity = mapper.selectOne( id ); 
    ...
    **/
    public List<WarehouseEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<WarehouseEntity> data = ExcelUtil.read(file.getInputStream(), type, WarehouseEntity.class, entityFieldTitleNameMap);
            batchCreate(data);
            return "ok";
        } catch (Exception e) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<WarehouseEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, WarehouseEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + WarehouseEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
            throw new HttpCodeException(500, e);
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public WarehouseEntity createOrUpdate(WarehouseEntity entity, List<String> updateFields) {
        if (null == entity) {
        ...
        if ( entity.getId() == null ) { 
            ...
            WarehouseEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
                UserContext.UserInfo currentUserInfo = UserContext.getCurrentUserInfo();
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(WarehouseEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
