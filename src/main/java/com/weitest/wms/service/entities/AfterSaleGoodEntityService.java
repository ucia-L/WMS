...
import com.weitest.wms.repository.entities.AfterSaleGoodEntityMapper;
...
/**
* auto generate AfterSaleGoodEntity ServiceImpl
*
* @author sys
*/
@Service
public class AfterSaleGoodEntityService extends AbstractService {
    @Resource
    private AfterSaleGoodEntityMapper mapper;
    @Resource
    ...
    public AfterSaleGoodEntityService() {
        entityFieldNameTitleMap.put("id", "入库货物编号");
        entityFieldTitleNameMap.put("入库货物编号", "id");
        entityFiledColumnNameMap.put("id", "id");
        entityFieldNameTitleMap.put("after_sale_id", "售后单号");
        entityFieldTitleNameMap.put("售后单号", "after_sale_id");
        entityFiledColumnNameMap.put("after_sale_id", "afterSaleId");
        ...
        entityFieldNameTitleMap.put("status", "入库状态");
        entityFieldTitleNameMap.put("入库状态", "status");
    ...
    **/
    ...
    public AfterSaleGoodEntity create(AfterSaleGoodEntity entity) {
        if (null == entity) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<AfterSaleGoodEntity> batchCreate(List<AfterSaleGoodEntity> entities) {
        ...
        // fill default value
        for (AfterSaleGoodEntity entity : entities) {
            paramValidate(entity);
        }
    ...
    **/
    @Transactional
    public AfterSaleGoodEntity update(AfterSaleGoodEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "AfterSaleGoodEntity");
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<AfterSaleGoodEntity> batchUpdate(List<AfterSaleGoodEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
        // updateFields为null时，默认全量更新
        List<AfterSaleGoodEntity> updateEntities = new ArrayList<>(entities.size());
        for (AfterSaleGoodEntity entity : entities) {
    ...
    **/
    ...
    public long delete( Long id ) { 
        ...
        AfterSaleGoodEntity entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("AfterSaleGood", "AfterSaleGood", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<AfterSaleGoodEntity>  entities = mapper.selectList(inQueryFilter);
        for (AfterSaleGoodEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
     ...
     * auto gen get method
     **/
    public AfterSaleGoodEntity get( Long id ) { 
        if ( id == null ) { 
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
        }
    ...
    **/
    public List<AfterSaleGoodEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<AfterSaleGoodEntity> data = ExcelUtil.read(file.getInputStream(), type, AfterSaleGoodEntity.class, entityFieldTitleNameMap);
            batchCreate(data);
            return "ok";
        } catch (Exception e) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<AfterSaleGoodEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, AfterSaleGoodEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + AfterSaleGoodEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
            throw new HttpCodeException(500, e);
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public AfterSaleGoodEntity createOrUpdate(AfterSaleGoodEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, AfterSaleGoodEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            AfterSaleGoodEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                paramValidate(entity);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(AfterSaleGoodEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
