...
import com.weitest.wms.repository.entities.InventoryTaskEntityMapper;
...
/**
* auto generate InventoryTaskEntity ServiceImpl
*
* @author sys
*/
@Service
public class InventoryTaskEntityService extends AbstractService {
    @Resource
    private InventoryTaskEntityMapper mapper;
    @Resource
    ...
    public InventoryTaskEntityService() {
        entityFieldNameTitleMap.put("id", "任务序号");
        entityFieldTitleNameMap.put("任务序号", "id");
        entityFiledColumnNameMap.put("id", "id");
        entityFieldNameTitleMap.put("createdTime", "任务创建时间");
        entityFieldTitleNameMap.put("任务创建时间", "createdTime");
        ...
        entityFieldNameTitleMap.put("id_tag", "任务编号");
        entityFieldTitleNameMap.put("任务编号", "id_tag");
    ...
    **/
    ...
    public InventoryTaskEntity create(InventoryTaskEntity entity) {
        if (null == entity) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<InventoryTaskEntity> batchCreate(List<InventoryTaskEntity> entities) {
        ...
        // fill default value
        for (InventoryTaskEntity entity : entities) {
            entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
            entity.setCreatedBy(StringUtils.isBlank(entity.getCreatedBy()) ? currentUserName : entity.getCreatedBy());
            paramValidate(entity);
        }
    ...
    **/
    @Transactional
    public InventoryTaskEntity update(InventoryTaskEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "InventoryTaskEntity");
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<InventoryTaskEntity> batchUpdate(List<InventoryTaskEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
        // updateFields为null时，默认全量更新
        List<InventoryTaskEntity> updateEntities = new ArrayList<>(entities.size());
        for (InventoryTaskEntity entity : entities) {
    ...
    **/
    ...
    public long delete( Long id ) { 
        ...
        InventoryTaskEntity entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("InventoryTask", "InventoryTask", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<InventoryTaskEntity>  entities = mapper.selectList(inQueryFilter);
        for (InventoryTaskEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
     ...
     * auto gen get method
     **/
    public InventoryTaskEntity get( Long id ) { 
        if ( id == null ) { 
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
        }
    ...
    **/
    public List<InventoryTaskEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<InventoryTaskEntity> data = ExcelUtil.read(file.getInputStream(), type, InventoryTaskEntity.class, entityFieldTitleNameMap);
            batchCreate(data);
            return "ok";
        } catch (Exception e) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<InventoryTaskEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, InventoryTaskEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + InventoryTaskEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
            throw new HttpCodeException(500, e);
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public InventoryTaskEntity createOrUpdate(InventoryTaskEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, InventoryTaskEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            InventoryTaskEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
                UserContext.UserInfo currentUserInfo = UserContext.getCurrentUserInfo();
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(InventoryTaskEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
