...
import com.weitest.wms.repository.entities.ApplicationEntityMapper;
...
/**
* auto generate ApplicationEntity ServiceImpl
*
* @author sys
*/
@Service
public class ApplicationEntityService extends AbstractService {
    @Resource
    ...
    public ApplicationEntityService() {
        ...
        entityFieldNameTitleMap.put("access_type", "权限种类");
        entityFieldTitleNameMap.put("权限种类", "access_type");
        entityFiledColumnNameMap.put("access_type", "accessType");
        entityFieldNameTitleMap.put("status", "申请状态");
        entityFieldTitleNameMap.put("申请状态", "status");
        entityFiledColumnNameMap.put("status", "status");
        entityFieldNameTitleMap.put("refuse_reason", "拒绝理由");
        entityFieldTitleNameMap.put("拒绝理由", "refuse_reason");
        entityFiledColumnNameMap.put("refuse_reason", "refuseReason");
        entityFieldNameTitleMap.put("apply_reason", "申请原因");
        entityFieldTitleNameMap.put("申请原因", "apply_reason");
        entityFiledColumnNameMap.put("apply_reason", "applyReason");
        entityFieldNameTitleMap.put("userId", "申请用户Id");
        entityFieldTitleNameMap.put("申请用户Id", "userId");
        entityFiledColumnNameMap.put("userId", "userId");
    }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<ApplicationEntity> batchCreate(List<ApplicationEntity> entities) {
    ...
    **/
    @Transactional
    public ApplicationEntity update(ApplicationEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "ApplicationEntity");
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<ApplicationEntity> batchUpdate(List<ApplicationEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
        // updateFields为null时，默认全量更新
        List<ApplicationEntity> updateEntities = new ArrayList<>(entities.size());
        for (ApplicationEntity entity : entities) {
    ...
    **/
    ...
    public long delete( Long id ) { 
        ...
        ApplicationEntity entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("Application", "Application", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<ApplicationEntity>  entities = mapper.selectList(inQueryFilter);
        for (ApplicationEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
    ...
    **/
    public List<ApplicationEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<ApplicationEntity> data = ExcelUtil.read(file.getInputStream(), type, ApplicationEntity.class, entityFieldTitleNameMap);
            batchCreate(data);
            return "ok";
        } catch (Exception e) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<ApplicationEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, ApplicationEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + ApplicationEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
            throw new HttpCodeException(500, e);
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public ApplicationEntity createOrUpdate(ApplicationEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, ApplicationEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            ApplicationEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
                entity.setUpdatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(ApplicationEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
