...
import com.weitest.wms.repository.entities.OutBoundWeighingEntityMapper;
...
/**
* auto generate OutBoundWeighingEntity ServiceImpl
*
* @author sys
*/
@Service
public class OutBoundWeighingEntityService extends AbstractService {
    @Resource
    private OutBoundWeighingEntityMapper mapper;
    @Resource
    ...
    public OutBoundWeighingEntityService() {
        entityFieldNameTitleMap.put("id", "称重编号");
        entityFieldTitleNameMap.put("称重编号", "id");
        ...
        entityFieldNameTitleMap.put("weighingWorkerID", "称重员工编号");
        entityFieldTitleNameMap.put("称重员工编号", "weighingWorkerID");
        entityFiledColumnNameMap.put("weighingWorkerID", "weighingWorkerID");
        entityFieldNameTitleMap.put("weighingWorkerName", "称重员工姓名");
        entityFieldTitleNameMap.put("称重员工姓名", "weighingWorkerName");
        entityFiledColumnNameMap.put("weighingWorkerName", "weighingWorkerName");
        entityFieldNameTitleMap.put("weighingTime", "称重时间");
        entityFieldTitleNameMap.put("称重时间", "weighingTime");
        entityFiledColumnNameMap.put("weighingTime", "weighingTime");
        entityFieldNameTitleMap.put("weight", "货物总重量");
        entityFieldTitleNameMap.put("货物总重量", "weight");
        entityFiledColumnNameMap.put("weight", "weight");
    }
    ...
    **/
    ...
    public OutBoundWeighingEntity create(OutBoundWeighingEntity entity) {
        if (null == entity) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<OutBoundWeighingEntity> batchCreate(List<OutBoundWeighingEntity> entities) {
        ...
        // fill default value
        for (OutBoundWeighingEntity entity : entities) {
            entity.setId(SnowflakeIdWorker.getInstance().nextId());
            paramValidate(entity);
        }
    ...
    **/
    @Transactional
    public OutBoundWeighingEntity update(OutBoundWeighingEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "OutBoundWeighingEntity");
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<OutBoundWeighingEntity> batchUpdate(List<OutBoundWeighingEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
        // updateFields为null时，默认全量更新
        List<OutBoundWeighingEntity> updateEntities = new ArrayList<>(entities.size());
        for (OutBoundWeighingEntity entity : entities) {
    ...
    **/
    ...
    public long delete( Long id ) { 
        ...
        OutBoundWeighingEntity entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("OutBoundWeighing", "OutBoundWeighing", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<OutBoundWeighingEntity>  entities = mapper.selectList(inQueryFilter);
        for (OutBoundWeighingEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
     ...
     * auto gen get method
     **/
    public OutBoundWeighingEntity get( Long id ) { 
        if ( id == null ) { 
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
        }
    ...
    **/
    public List<OutBoundWeighingEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<OutBoundWeighingEntity> data = ExcelUtil.read(file.getInputStream(), type, OutBoundWeighingEntity.class, entityFieldTitleNameMap);
            batchCreate(data);
            return "ok";
        } catch (Exception e) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<OutBoundWeighingEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, OutBoundWeighingEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + OutBoundWeighingEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
            throw new HttpCodeException(500, e);
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public OutBoundWeighingEntity createOrUpdate(OutBoundWeighingEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, OutBoundWeighingEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            OutBoundWeighingEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                paramValidate(entity);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(OutBoundWeighingEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
