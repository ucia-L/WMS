...
import com.weitest.wms.repository.entities.MessageeEntityMapper;
...
/**
* auto generate MessageeEntity ServiceImpl
*
* @author sys
*/
@Service
public class MessageeEntityService extends AbstractService {
    @Resource
    private MessageeEntityMapper mapper;
    @Resource
    ...
    public MessageeEntityService() {
        entityFieldNameTitleMap.put("id", "消息编号");
        entityFieldTitleNameMap.put("消息编号", "id");
        entityFiledColumnNameMap.put("id", "id");
        entityFieldNameTitleMap.put("createdTime", "时间");
        entityFieldTitleNameMap.put("时间", "createdTime");
        entityFiledColumnNameMap.put("createdTime", "createdTime");
        entityFieldNameTitleMap.put("text", "内容");
        entityFieldTitleNameMap.put("内容", "text");
        entityFiledColumnNameMap.put("text", "text");
        entityFieldNameTitleMap.put("sender", "发送人");
        entityFieldTitleNameMap.put("发送人", "sender");
        entityFiledColumnNameMap.put("sender", "sender");
        entityFieldNameTitleMap.put("status", "状态");
        entityFieldTitleNameMap.put("状态", "status");
        entityFiledColumnNameMap.put("status", "status");
        entityFieldNameTitleMap.put("category", "类别");
        entityFieldTitleNameMap.put("类别", "category");
        entityFiledColumnNameMap.put("category", "category");
        entityFieldNameTitleMap.put("subject", "主题");
        entityFieldTitleNameMap.put("主题", "subject");
        entityFiledColumnNameMap.put("subject", "subject");
    }
    ...
    **/
    ...
    public MessageeEntity create(MessageeEntity entity) {
        if (null == entity) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<MessageeEntity> batchCreate(List<MessageeEntity> entities) {
        ...
        // fill default value
        for (MessageeEntity entity : entities) {
            entity.setId(SnowflakeIdWorker.getInstance().nextId());
            entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
            paramValidate(entity);
        }
    ...
    **/
    @Transactional
    public MessageeEntity update(MessageeEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "MessageeEntity");
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<MessageeEntity> batchUpdate(List<MessageeEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
        // updateFields为null时，默认全量更新
        List<MessageeEntity> updateEntities = new ArrayList<>(entities.size());
        for (MessageeEntity entity : entities) {
    ...
    **/
    ...
    public long delete( Long id ) { 
        ...
        MessageeEntity entity = mapper.selectOne( id ); 
    ...
    **/
     ...
     * auto gen get method
     **/
    public MessageeEntity get( Long id ) { 
        if ( id == null ) { 
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
        }
    ...
    **/
    public List<MessageeEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<MessageeEntity> data = ExcelUtil.read(file.getInputStream(), type, MessageeEntity.class, entityFieldTitleNameMap);
            batchCreate(data);
            return "ok";
        } catch (Exception e) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<MessageeEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, MessageeEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + MessageeEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
            throw new HttpCodeException(500, e);
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public MessageeEntity createOrUpdate(MessageeEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, MessageeEntity.class);
        }
        ...
        if ( entity.getId() == null ) { 
            ...
            MessageeEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                entity.setCreatedTime(ZonedDateTime.now(ZoneId.of("UTC")));
                paramValidate(entity);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(MessageeEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
