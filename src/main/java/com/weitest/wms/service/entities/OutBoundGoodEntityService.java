...
import com.weitest.wms.repository.entities.OutBoundGoodEntityMapper;
...
/**
* auto generate OutBoundGoodEntity ServiceImpl
*
* @author sys
*/
@Service
public class OutBoundGoodEntityService extends AbstractService {
    @Resource
    private OutBoundGoodEntityMapper mapper;
    @Resource
    ...
    public OutBoundGoodEntityService() {
        entityFieldNameTitleMap.put("id", "出库货物编号");
        entityFieldTitleNameMap.put("出库货物编号", "id");
        entityFiledColumnNameMap.put("id", "id");
        entityFieldNameTitleMap.put("outbound_order_id", "出库订单编号");
        entityFieldTitleNameMap.put("出库订单编号", "outbound_order_id");
        entityFiledColumnNameMap.put("outbound_order_id", "outboundOrderId");
        ...
        entityFieldNameTitleMap.put("status", "货物状态");
        entityFieldTitleNameMap.put("货物状态", "status");
        entityFiledColumnNameMap.put("status", "status");
        entityFieldNameTitleMap.put("weight", "重量");
        entityFieldTitleNameMap.put("重量", "weight");
    ...
    **/
    ...
    public OutBoundGoodEntity create(OutBoundGoodEntity entity) {
        if (null == entity) {
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<OutBoundGoodEntity> batchCreate(List<OutBoundGoodEntity> entities) {
        ...
        // fill default value
        for (OutBoundGoodEntity entity : entities) {
            paramValidate(entity);
        }
    ...
    **/
    @Transactional
    public OutBoundGoodEntity update(OutBoundGoodEntity entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "OutBoundGoodEntity");
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<OutBoundGoodEntity> batchUpdate(List<OutBoundGoodEntity> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
        // updateFields为null时，默认全量更新
        List<OutBoundGoodEntity> updateEntities = new ArrayList<>(entities.size());
        for (OutBoundGoodEntity entity : entities) {
    ...
    **/
    ...
    public long delete( Long id ) { 
        ...
        OutBoundGoodEntity entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("OutBoundGood", "OutBoundGood", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<OutBoundGoodEntity>  entities = mapper.selectList(inQueryFilter);
        for (OutBoundGoodEntity entity : entities) {
            relationInnerService.onDelete(entity);
        }
     ...
     * auto gen get method
     **/
    public OutBoundGoodEntity get( Long id ) { 
        if ( id == null ) { 
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
        }
    ...
    **/
    public List<OutBoundGoodEntity> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<OutBoundGoodEntity> data = ExcelUtil.read(file.getInputStream(), type, OutBoundGoodEntity.class, entityFieldTitleNameMap);
            batchCreate(data);
            return "ok";
        } catch (Exception e) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<OutBoundGoodEntity> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, OutBoundGoodEntity.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + OutBoundGoodEntity.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
            throw new HttpCodeException(500, e);
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public OutBoundGoodEntity createOrUpdate(OutBoundGoodEntity entity, List<String> updateFields) {
        if (null == entity) {
        ...
        if ( entity.getId() == null ) { 
            ...
            OutBoundGoodEntity existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                paramValidate(entity);
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(OutBoundGoodEntity entity, List<String> updateFields, AbstractQueryFilter filter) {
