...
import com.weitest.wms.repository.entities.LCAPLogicViewMappingMapper;
...
/**
* auto generate LCAPLogicViewMapping ServiceImpl
*
* @author sys
*/
@Service
public class LCAPLogicViewMappingService extends AbstractService {
    @Resource
    private LCAPLogicViewMappingMapper mapper;
    @Resource
    ...
    public LCAPLogicViewMappingService() {
        entityFieldNameTitleMap.put("id", "主键");
        entityFieldTitleNameMap.put("主键", "id");
        entityFiledColumnNameMap.put("id", "id");
        entityFieldNameTitleMap.put("logicIdentifier", "逻辑标识");
        entityFieldTitleNameMap.put("逻辑标识", "logicIdentifier");
        entityFiledColumnNameMap.put("logicIdentifier", "logicIdentifier");
        entityFieldNameTitleMap.put("resourceName", "资源路径");
        entityFieldTitleNameMap.put("资源路径", "resourceName");
        entityFiledColumnNameMap.put("resourceName", "resourceName");
        entityFieldNameTitleMap.put("resourceType", "资源类型");
        entityFieldTitleNameMap.put("资源类型", "resourceType");
        entityFiledColumnNameMap.put("resourceType", "resourceType");
        entityFieldNameTitleMap.put("group", "逻辑与资源绑定的分组关系");
        entityFieldTitleNameMap.put("逻辑与资源绑定的分组关系", "group");
        entityFiledColumnNameMap.put("group", "group");
        entityFieldNameTitleMap.put("changeTime", "创建时间");
        entityFieldTitleNameMap.put("创建时间", "changeTime");
        entityFiledColumnNameMap.put("changeTime", "changeTime");
    }
    ...
    **/
    ...
    public LCAPLogicViewMapping create(LCAPLogicViewMapping entity) {
        if (null == entity) {
            throw new HttpCodeException(400, "create param is required");
        }
        // fill default value
        entity.setId(SnowflakeIdWorker.getInstance().nextId());
        entity.setLogicIdentifier(getOrDefault(entity.getLogicIdentifier(), ""));
        entity.setResourceName(getOrDefault(entity.getResourceName(), ""));
        entity.setResourceType(getOrDefault(entity.getResourceType(), ""));
        entity.setGroup(getOrDefault(entity.getGroup(), 0L));
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<LCAPLogicViewMapping> batchCreate(List<LCAPLogicViewMapping> entities) {
        ...
        // fill default value
        for (LCAPLogicViewMapping entity : entities) {
            entity.setId(SnowflakeIdWorker.getInstance().nextId());
            entity.setLogicIdentifier(getOrDefault(entity.getLogicIdentifier(), ""));
            entity.setResourceName(getOrDefault(entity.getResourceName(), ""));
            entity.setResourceType(getOrDefault(entity.getResourceType(), ""));
    ...
    **/
    @Transactional
    public LCAPLogicViewMapping update(LCAPLogicViewMapping entity, List<String> updateFields) {
        if (null == entity) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_REQUIRED.code, "LCAPLogicViewMapping");
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public List<LCAPLogicViewMapping> batchUpdate(List<LCAPLogicViewMapping> entities, List<String> updateFields) {
        if (null == entities || entities.isEmpty()) {
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_NOTHING_TODO.code);
        }
        // updateFields为null时，默认全量更新
        List<LCAPLogicViewMapping> updateEntities = new ArrayList<>(entities.size());
        for (LCAPLogicViewMapping entity : entities) {
    ...
    **/
    ...
    public long delete( Long id ) { 
        ...
        LCAPLogicViewMapping entity = mapper.selectOne( id ); 
    ...
    **/
    ...
    public long batchDelete(List<Long> ids) {
        ...
        AbstractQueryFilter inQueryFilter = new InQueryFilter()
            .left(new ColumnQueryFilter("LCAPLogicViewMapping", "LCAPLogicViewMapping_79f632", "id", "id"))
            .right(new ListLiteralQueryFilter(ids));
        CommonFunctionUtil.preHandleQueryExpression(inQueryFilter, entityFiledColumnNameMap);
        List<LCAPLogicViewMapping>  entities = mapper.selectList(inQueryFilter);
        for (LCAPLogicViewMapping entity : entities) {
            relationInnerService.onDelete(entity);
        }
     ...
     * auto gen get method
     **/
    public LCAPLogicViewMapping get( Long id ) { 
        if ( id == null ) { 
            throw new HttpCodeException(400, ErrorCodeEnum.PARAM_PRIMARY_KEY_REQUIRED.code);
        }
    ...
    **/
    public List<LCAPLogicViewMapping> list(AbstractQueryFilter queryFilter) {
        if (null == queryFilter) {
    ...
    **/
    ...
    public String importFile(MultipartFile file) {
        ...
        try {
            List<LCAPLogicViewMapping> data = ExcelUtil.read(file.getInputStream(), type, LCAPLogicViewMapping.class, entityFieldTitleNameMap);
            batchCreate(data);
            return "ok";
        } catch (Exception e) {
    ...
    **/
    public ResponseEntity<org.springframework.core.io.Resource> export(AbstractQueryFilter queryFilter, String fields, HttpServletRequest request) {
        try {
            ...
            List<LCAPLogicViewMapping> data = list(queryFilter);
            String storeFilePath = ExcelUtil.write(data, LCAPLogicViewMapping.class, exportFieldMap);
            ...
            return ResponseEntity.ok()
                ...
                .header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=\"" + LCAPLogicViewMapping.class.getSimpleName() + ".xlsx\"")
                .body(resource);
        } catch (Exception e) {
            throw new HttpCodeException(500, e);
        }
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public LCAPLogicViewMapping createOrUpdate(LCAPLogicViewMapping entity, List<String> updateFields) {
        if (null == entity) {
        ...
        if ( entity.getId() == null ) { 
            ...
            LCAPLogicViewMapping existEntity = mapper.selectOne(entity.getId()); 
            if (null == existEntity) {
                // insert
                entity.setLogicIdentifier(getOrDefault(entity.getLogicIdentifier(), ""));
                entity.setResourceName(getOrDefault(entity.getResourceName(), ""));
    ...
    **/
    @Transactional(rollbackFor = Exception.class)
    public long updateBy(LCAPLogicViewMapping entity, List<String> updateFields, AbstractQueryFilter filter) {
